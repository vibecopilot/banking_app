...
(function($) {
  ...
  $.paletteColorPicker = function( el, options ) {
    ...
    //  plugin.settings.colors = ["#D50000","#304FFE","#00B8D4","#00C853","#eb5768","#FF1744","#6F0564","#FFCC00","#AF4FA2","#800000","#248108","#0033CC"];
      ...
      .on(click_handler, '.'+ns+'-bubble span.swatch', function(e){
        ...
        if ( $(e.target).is('.clear') ) {
          $(this).parents(".attachcolor").find($button).removeAttr('style');
          col = '';
        } else {
          $(this).addClass('active');
          $(this).parents(".attachcolor").find($button).css('background', col);
        }
        ...
        if( plugin.settings.set_background === false ) {
          $(this).parents(".attachcolor").find('[name="' + $button.attr('data-target') + '"]').val(name);
        } else {
          //console.log("setting bg");
          $(this).closest('[name="' + $button.attr('data-target') + '"]').css({'background-color' : col});
        }
