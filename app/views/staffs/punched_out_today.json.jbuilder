...
json.staffs do
  json.array! @staffs do |staff|
    ...
    # Get today's completed attendance (punched in and out)
    today_attendance = staff.attendances.where('DATE(punched_in_at) = ? AND punched_out_at IS NOT NULL', Date.current).first
    ...
    if today_attendance.present?
      json.today_attendance do
        ...
        json.punched_out_time today_attendance.punched_out_time
