# app/views/visitors/visitors_list.json.jbuilder
...
json.lotus_token visitor.lotus_token
json.start_date visitor.start_date
json.end_date visitor.end_date
...
if visitor.visitor_staff_category.present?
  json.visitor_staff_category do
    json.id visitor.visitor_staff_category.id
    json.name visitor.visitor_staff_category.name
  end
else
  json.visitor_staff_category []
end




json.logs Visitor.where(contact_no: visitor.contact_no) do |log|
  json.id log.id
  json.name log.name
  json.contact_no log.contact_no
  json.purpose log.purpose
  json.site_id log.site_id
  json.coming_from log.coming_from
  json.vehicle_number log.vehicle_number
  json.expected_date log.expected_date
  json.verified log.verified
  json.start_pass visitor.start_pass
  json.end_pass visitor.end_pass
  json.pass_number visitor.pass_number
  json.created_at visitor.created_at
  ...
  json.created_by_name created_by_user&.slice(:firstname, :lastname)
  json.user_type created_by_user&.user_type
  json.visits_log log.visitor_visits do |visit|
    json.visitor_id visit.visitor_id if visit.check_in.present?
    json.check_in visit.check_in if visit.check_in.present? 
    json.check_out visit.check_out if visit.check_out.present?
  end
...
if @parent.present?
  json.profile_picture @parent.profile_pic&.image&.url
  json.parent_id @parent.id
else
  json.profile_picture visitor.profile_pic&.image&.url
end

json.visitor_files @visitor_files do |doc|
  json.id doc.id
